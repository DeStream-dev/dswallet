...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("destream"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("destream://"))
    {
        uri.replace(0, 12, "destream:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DeStream.lnk";
}
...
{
    return GetAutostartDir() / "destream.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DeStream-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  destream-qt [" + tr("command-line options") + "]                     " + "\n";
